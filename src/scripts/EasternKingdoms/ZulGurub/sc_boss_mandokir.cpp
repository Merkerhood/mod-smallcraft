...
struct boss_mandokir : public BossAI
{
    ...
    {
        LOG_DEBUG("module.SmallCraft.ai", "SmallCraft:boss_mandokir:: JustEngagedWith");
        ...
        events.ScheduleEvent(EVENT_WATCH_PLAYER, 18s, 28s);
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_WATCH_PLAYER:
                    if (Unit* player = SelectTarget(SelectTargetMethod::Random, 0, 150, true, false))
                    {
...
{
    RegisterCreatureAI(boss_mandokir);
    // new npc_ohgan();
